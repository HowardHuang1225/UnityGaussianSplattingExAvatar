...
### 純二進位 LBS 流程 (Pure Binary Mode)

為了優化專案打包、降低檔案體積以及提升多人物場景的效能，專案支援 **Pure Binary LBS** 流程：

1.  **設計目的**：
    - **減少檔案大小**：避開體積龐大的 ONNX 模型文件，改用精簡的 `.bytes` 二進位數據。
    - **提升加載速度**：二進位數據可直接讀取至記憶體與 GPU 緩衝區，無需經過 ONNX Runtime 的解析與初始化（Inference Engine 暖機）。
    - **多人物擴展性**：在需要大量渲染人物的場景下，減少 CPU/GPU 在推論層面的負擔。
2.  **實作細節**：
    - **切換開關**：在 `HumanGaussianInference` 元件中取消勾選 `Use ONNX` 即可啟用。
    - **預烘焙數據 (Baked Data)**：系統會讀取從 Exavatar 訓練好的 **pose-independent MLP inference** 中導出的 **static attributes**（包含 Mean3d, Scale, RGB 以及 Neutral Pose 的骨架數據和 skinning-weight），並配合 LBS Compute Shader 進行即時變形。
3.  **使用場景**：
    - 需要實際建置執行檔 (Build in executable) 以優化效能與體積。
    - 場景中存在多個不同人物，且需要各自執行獨立的動畫邏輯。

4.  **表情處理說明 (Facial Expressions)**：
    - **ONNX 模式(refine版本)**：全動態表情支援。MLP 模型會根據輸入的 `expr` 即時計算高斯點偏移，展現細微面部肌肉變化（如皺眉、笑）。
    - **Pure Binary 模式**：受限於靜態屬性，僅支援由骨架驅動的表情（如 **張嘴/下巴移動 `jaw_pose`**、**眼球轉動 `eye_pose`**）。其餘細微表情將維持在烘焙數據時的初始狀態 (與 static onnx 相同)。
...
into Unity is just "MIT license", you need to separately consider _how_ did you get your Gaussian Splat PLY files.
